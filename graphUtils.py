...
#FIX URLs in graph for matches (mid)???
    gvFil.write(ind['RGDid'] + '[URL="/graph?wid='+str(ind['_id'])+'" target=_blank, label=<'+lab2+'>, '+attrs+'];')
    gvFil.write("\n")
    return        
...
#No URL for family-graphs
        gvFil.write(fam['RGDid'] + '[label=<'+txt+'>, style=filled, shape="note"];')
        gvFil.write("\n")
        for ch in fam['children']:
            gvFil.write(fam['RGDid'] + '->' + mapPersId[ch])
            gvFil.write("\n")
        if 'wife' in fam and fam['wife']:
            gvFil.write(mapPersId[fam['wife']] + '->' + fam['RGDid'])
            gvFil.write("\n")
        if 'husb' in fam and fam['husb']:
            gvFil.write(mapPersId[fam['husb']] + '->' + fam['RGDid'])
            gvFil.write("\n")
    gvFil.write( "}\n" )
    gvFil.close()
