...
// AA0 lock table in Web-environment
	$result = mysqli_query($link,"LOCK TABLES sour WRITE");
	if(!$result)
	{
		echo "LOCK av sour fungerande inte".mysqli_error($link);
	}
...
//	$SQL="TRUNCATE sour"; 
	...
	if(!$result)
	{
		echo $SQL."Tömningen av sour fungerande inte".mysqli_error($link);
	}
//	
	$len=0;
	$txt='';
...
//				
				...
				if(!$result)
				{	
					echo $SQL."fungerande inte".mysqli_error($link)."<br/>";
				}
				$txt="";
			}
		}		
...
//	
	...
	if(!$result)
	{
	  echo "UNLOCK av sour fungerande inte".mysqli_error($link);
	}
}
else
{
...
//
	if($larmd > 0)
	{
		$fileun=$directory . "Note.txt";
		$handun=fopen($fileun,"w");
		fwrite($handun," \r\n");
		fwrite($handun,"NOTERING OM OPRECISERADE DATUM. \r\n");
		fwrite($handun," \r\n");
//
		$brytr = 0;
		if($larmd > 0) {
			...
			foreach($lrmlistd as $lrmradd) {
				...
				fwrite($handun,$lrmradd." \r\n");
			}
		}
		fwrite($handun," \r\n");
		fwrite($handun,"Om möjlighet finns, bör datum anges i exakt datumformat. \r\n");
		fwrite($handun," \r\n");
		fclose($handun);
	}
//	
...
{
	echo "<br/>";
	echo '* * * Lista Note.txt med opreciserade datum har skapats. <br/>';
}
//
if($larmant > 0) {
	echo "<br/>";
